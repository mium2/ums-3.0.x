...
public class PushSend extends BaseSend {
    ...
    public Map<String,Object> send(ReqUmsSendVo reqUmsSendVo, String requestUri, Locale locale) throws Exception{
        ...
        // STEP 1 :  공통 필수 항목 체크 
    	// 라이센스 체크 (푸시 발송 경우만)
        if(!LicenseValidator.getInstance().chkAppid(reqUmsSendVo.getAPP_ID())) throw new LicenseException("앱아이디 :"+reqUmsSendVo.getAPP_ID());

        // 앱ID 체크
        if(StringUtils.isBlank(reqUmsSendVo.getAPP_ID())) throw new ValidationException("[APP_ID]", locale);

        // STEP 2 :  푸시메세지 체크
        if(StringUtils.isBlank(reqUmsSendVo.getPUSH_MSG())) throw new ValidationException("[PUSH_MSG]", locale);
...
//		   		resultBodyMap= umsCsvMemberSendService.umsPushCsvMemberSend(umsSendMsgBean);
//		   		break;
		   		throw new ValidationException("[NOT SUPPORTED TARGET SEND TYPE]", locale);
		   	case NC:	// UMS비회원 CSV - CSV파일 필수. #{아이디} #{핸드폰번호} #{이름} 필수
		   		resultBodyMap = umsCsvSendService.umsPushCsvSend(umsSendMsgBean);
		   		break;
		   	case NM:	// UMS비회원 - CUIDS파라터 필수. {"아이디":["핸드폰번호","이름"},...}
	            ...
	            resultBodyMap = umsSendService.umsPushSend(umsSendMsgBean);
		   		break;
		   	default:
	        	if(StringUtils.isBlank(reqUmsSendVo.getCUIDS())) throw new ValidationException("[CUIDS]", locale);
	            resultBodyMap = umsSendMemberService.umsMemberUserPushSend(umsSendMsgBean,locale);
		   		break;
        }
