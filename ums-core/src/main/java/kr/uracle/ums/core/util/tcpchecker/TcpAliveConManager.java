 ...
 * 해당 TCP 메니저는 등록된 UPMC 시스템을 주기적으로 감시하여 Loadbalancing과 Failover처리를 한다.
 */
@Service
public class TcpAliveConManager{
    ...
    @Autowired
    public TcpAliveConManager(@Value("${PUSH.UPMC.LOCAL.URLS:}") String upmc_localhot,
                              @Value("${PUSH.UPMC.FAILOVER.URLS:}") String upmc_failover_hosts,
                              @Value("${PUSH.UPMC.CHECK.SECOND:10}") String chkLoopTime){
        int chkLoopSecond = Integer.parseInt(chkLoopTime);
        List<String> failoverHostNames = new ArrayList<String>();
        if(upmc_failover_hosts.indexOf(",")>0){
            String[] chkHostNameArray = upmc_failover_hosts.split(",");
            for (int i = 0; i < chkHostNameArray.length; i++) {
                String chkHostName = chkHostNameArray[i].trim();
                if(!"/".equals(chkHostName.substring(chkHostName.length()-1))){
                    chkHostName = chkHostName+"/";
                }
                failoverHostNames.add(chkHostName);
            }
        }else{
            if(!"".equals(upmc_failover_hosts)){
                String chkHostName = upmc_failover_hosts;
                if(!"/".equals(chkHostName.substring(chkHostName.length()-1))){
                    chkHostName = chkHostName+"/";
                }
                failoverHostNames.add(chkHostName);
        ...
        init(upmc_localhot,failoverHostNames,chkLoopSecond);
    }
     ...
     * @param checkLoopTime
     */
    public void init(String localHostName, List<String> failoverHostNames, int checkLoopTime){
        this.localhostName = localHostName;
        failoverHostNames.add(localHostName);
        connetionRetryTime = checkLoopTime*1000;
    ...
    public synchronized String getConHostName() throws Exception{
        ...
        if(aliveHostNameList.size()==0){
            throw new Exception("There is no session is connected upmc host.");
        }
    ...
    public List<String> getBadHostNameList() {
        List<String> cloneBadHostNames = new ArrayList<String>();
        cloneBadHostNames.addAll(badHostNameList);
        if(!isLocalhostAlive){
            cloneBadHostNames.add(localhostName);
        }
        return cloneBadHostNames;
    }
    ...
    public List<String> getAliveHostNameList() {
        ...
        if(isLocalhostAlive){
            cloneAliveHostNames.add(localhostName);
        }
